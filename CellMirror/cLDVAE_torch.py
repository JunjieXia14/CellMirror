...
#cLDVAE_Pytorch_version
class cLDVAE(nn.Module):
    def __init__(
        self,
        args,
        n_input: int,
    ...
    def inference(self, tg, bg):
        ...
        tg_outputs = self.linear_decoder(torch.cat([tg_z, tg_s], dim=1))
        zeros = torch.zeros_like(bg_s)
        bg_outputs = self.linear_decoder(torch.cat([bg_z, zeros], dim=1))
    ...
    def get_cLDVAE_loss(self, tg, bg):
        ...
        cLDVAE_loss = torch.mean(reconstruction_loss) + self.beta * torch.mean(kl_loss) + self.gamma * torch.mean(tc_loss) + torch.mean(discriminator_loss)

        return torch.mean(reconstruction_loss), self.beta * torch.mean(kl_loss), self.gamma * torch.mean(tc_loss), torch.mean(discriminator_loss), cLDVAE_loss
    ...
    def predict(self, dataloader):
        ...
        for batch_idx, (tg, bg) in enumerate(dataloader):
            ...
            noContamination_output = self.linear_decoder( torch.cat([outputs["tg_z"], zeros], dim=1) )
    ...
    def fit(self, train_loader, total_loader):
        ...
        for epoch in range( 1, self.args.max_epoch + 1 ):
            ...
            for batch_idx, (tg, bg) in enumerate(train_loader):
                ...
                _, _, _, _, loss= self.get_cLDVAE_loss(tg, bg)
    ...
    def get_loadings(self) -> np.ndarray:
        if len(self.n_hidden_de) > 0:
            loadings = (self.linear_decoder.factor_regressor.fc_layers[0][0].weight).t()

            if len(self.n_hidden_de) > 1:
                for i in range(1, len(self.n_hidden_de)):
                    loadings = torch.mm( loadings, torch.t(self.linear_decoder.factor_regressor.fc_layers[i][0].weight) )

            loadings = torch.mm( loadings, (self.linear_decoder.decoder_x.weight).t() )

            loadings = loadings.t()

            loadings = ( loadings.detach().cpu().numpy() )
        else:
            loadings = self.linear_decoder.factor_regressor.fc_layers[0][0].weight
